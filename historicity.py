...
Classes:
    ...
    `Analyze`: plots or exports analysis of the user's choice
"""
...
class Analyze:
    ...
    def track_keywords_perdirectory(self, keywords:list[str], rowname="placeholder", csv_name="text_analysis.csv"):
        """
    ...
    def track_keywords_perfile(self, keywords:list[str], rowname="placeholder", csv_name="text_analysis.csv"):
        """
        Ennumerates instances of desirable indexing terms within each text files in specified directory and appends the collected data into a new CSV. If none specified, the default directory will be chosen.
        ...
        ## dict comprehension
        statistics = {k:v for (k,v) in zip(keywords,[0 for i in range(len(keywords))])}
        csv_writer.writerow(["number"]+[rowname]+list(statistics.keys()))
        ...
        for file in txt_files:
            ...
            csv_writer.writerow([p]+[str(file)]+list(statistics.values()))
            p += 1
